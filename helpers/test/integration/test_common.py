...
def _with_reply_process(endpoint, responses, queue, handle_subscriptions=False):
    while responses:
        ...
        for received_msg in received_msgs:
            ...
            if client_message.HasField('message_id') or handle_subscriptions:
...
@contextmanager
def reply(endpoint, responses, handle_subscriptions=False):
    if not isinstance(responses, list):
        responses = [responses]

    queue = Queue()
    p = Thread(target=_with_reply_process, args=(endpoint, responses, queue),
               kwargs={'handle_subscriptions': handle_subscriptions})
    p.start()
