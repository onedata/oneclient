...
#define CRYPTO_FREE(M, X)                                                      \
    if (X) {                                                                   \
        M##_free(X);                                                           \
        X = NULL;                                                              \
    }

constexpr const char *X509_USER_PROXY_ENV = "X509_USER_PROXY",
...
namespace {
inline std::string GLOBUS_PROXY_PATH()
{
    return "/tmp/x509up_u" + std::to_string(getuid());
}
...
string getPasswd(const string &prompt, int max_size)
{
    ...
    if (f) {
        while (fread(passwd + i++, 1, 1, f) == 1 && passwd[i - 1] != endline &&
            i < max_size)
            ;
        fclose(f);
    }
...
int pass_cb(char *buf, int size, int rwflag, void *u)
{
    ...
    string passphrase =
        getPasswd("Enter GRID pass phrase for your identity: ", size);
...
template <typename T> inline std::pair<T, T> make_pair(const T &elem)
{
...
const std::vector<std::pair<string, string>> &GSIHandler::getCertSearchPath()
{
    ...
    if (searchPath.empty()) {
        searchPath.push_back(make_pair(GLOBUS_PROXY_PATH()));
        searchPath.push_back(
            std::make_pair(absPathRelToHome(GLOBUS_PEM_CERT_PATH),
                absPathRelToHome(GLOBUS_PEM_KEY_PATH)));
        searchPath.push_back(make_pair(absPathRelToHome(GLOBUS_P12_PATH)));
        searchPath.push_back(
            std::make_pair(absPathRelToHome(GLOBUS_DIR_PATH), string()));
    }
...
std::pair<string, string> GSIHandler::findUserCertAndKey()
{
    ...
    for (const auto &elem : getCertSearchPath()) {
        if (exists(elem.first) && is_directory(elem.first) &&
            elem.second.empty()) {
...
std::pair<string, string> GSIHandler::getUserCertAndKey()
{
    ...
    if (m_context.lock()->options()->has_peer_certificate_file())
        return make_pair(absPathRelToHome(
            m_context.lock()->options()->get_peer_certificate_file()));

    if (getenv(X509_USER_PROXY_ENV) &&
        boost::filesystem::exists(getenv(X509_USER_PROXY_ENV)))
    ...
    // Any found path can be overriden by user's envs, provided it's not a proxy
    // path
    if (certAndKey.first != GLOBUS_PROXY_PATH()) {
        if (getenv(X509_USER_CERT_ENV) &&
            boost::filesystem::exists(getenv(X509_USER_CERT_ENV)))
            certAndKey.first = getenv(X509_USER_CERT_ENV);
        if (getenv(X509_USER_KEY_ENV) &&
            boost::filesystem::exists(getenv(X509_USER_KEY_ENV)))
...
void GSIHandler::validateProxyCert()
{
    ...
    string cPathMode = "", cPathMode1 = "",
           debugStr = (m_debug ? " -debug" : "");
    struct stat buf;
    ...
    if (userCert == "") {
        throw AuthException{"Couldn't find valid credentials.\n"
                            "The user cert could not be found in: \n"
                            "   1) env. var. " +
            std::string{X509_USER_PROXY_ENV} + "\n"
                                               "   2) proxy crt. " +
            GLOBUS_PROXY_PATH() + "\n"
                                  "   3) env. var. " +
            std::string{X509_USER_CERT_ENV} + "\n"
                                              "   4) $HOME/" +
            std::string{GLOBUS_PEM_CERT_PATH} + "\n"
                                                "   5) $HOME/" +
            std::string{GLOBUS_P12_PATH} + "\n"
                                           "   6) $HOME/" +
            std::string{GLOBUS_DIR_PATH} + "/*.p12\n"
                                           "   6) $HOME/" +
            std::string{GLOBUS_DIR_PATH} + "/*.pem + .key"};
    }

    if (stat(userCert.c_str(), &buf) != 0) {
        throw AuthException{"Couldn't find valid credentials.\n"
                            "You have no permissions to read user cert file: " +
            userCert};
    }

    if ((buf.st_mode & ACCESSPERMS) > 0644 ||
        (buf.st_mode & (S_IWGRP | S_IXGRP | S_IWOTH | S_IXOTH))) {
        throw AuthException{"Couldn't find valid credentials.\n"
                            "Your user cert file: " +
            userCert + " is to permissive. Maximum of 0644."};
    }
    ...
    if (userKey == "") {
        throw AuthException{
            ...
            "   1) env. var. " +
            std::string{X509_USER_KEY_ENV} + "\n"
                                             "   2) $HOME/" +
            std::string{GLOBUS_PEM_KEY_PATH} + "\n"
                                               "   3) $HOME/" +
            std::string{GLOBUS_P12_PATH}};
    }
    ...
    if (stat(userKey.c_str(), &buf) != 0) {
        throw AuthException{
            "Couldn't find valid credentials to generate a proxy.\n"
            "You have no permissions to read user key file: " +
            userKey};
    }

    if ((buf.st_mode & ACCESSPERMS) > 0600 ||
        (buf.st_mode & (S_IRWXO | S_IRWXG))) {
        throw AuthException{
            "Couldn't find valid credentials to generate a proxy.\n"
            "Your user key file: " +
            userKey + " is to permissive. Maximum of 0600."};
    }
    ...
    if (BIO_read_filename(file, userKey.c_str()) <= 0) {
        ...
        throw AuthException{
            "Couldn't find valid credentials to generate a proxy.\n"
            "Failed to read your key file: " +
            userKey + " (try checking read permissions)."};
    }
    ...
    EVP_PKEY *key = PEM_read_bio_PrivateKey(
        file, nullptr, pass_cb, nullptr); // Try to read key faile as .pem
    X509 *cert = nullptr;
    ...
    if (key == nullptr) {
        ...
        if (ERR_GET_REASON(e) == 100) { // Invalid passphrase
            LOG(ERROR) << "GSI Handler: parsing userKey as PEM failed due to "
                          "inavlid passphrase";
            CRYPTO_FREE(BIO, file);
            ...
            if (p12 == nullptr) {
                ...
                throw AuthException{"Invalid .pem or .p12 certificate file: " +
                    userKey + " " + MSG_DEBUG_INFO(m_debug) + "\n" +
                    (m_debug ? std::string{ERR_error_string(e2, nullptr)} + "\n"
                             : std::string{}) +
                    ERR_reason_error_string(e2)};
            }
            else {
                string pswd = getPasswd(
                    "Enter MAC pass phrase for your identity: ", 1024);
                if (!PKCS12_parse(p12, pswd.c_str(), &key, &cert, &ca)) {
                    unsigned long e1 = ERR_get_error();
                    if (ERR_GET_REASON(e1) == 113) { // MAC Validation error
                        LOG(ERROR) << "GSI Handler: parsing userKey as PKCS12 "
                                      "failed due to inavlid passphrase";

                        CRYPTO_FREE(BIO, file);
                        throw AuthException{
                            "Entered key passphrase is invalid."};
                    }
                    else {
                        LOG(ERROR)
                        ...
                        CRYPTO_FREE(BIO, file);
                        throw AuthException{
                            "Cannot parse .p12 file. " +
                            MSG_DEBUG_INFO(m_debug) +
                            (m_debug
                                    ? "\n" + std::string{ERR_error_string(
                                                 e1, nullptr)}
                                    : std::string{})};
                    }
                }
    ...
    if (X509_cmp_current_time(notAfter) <= 0) {
        ...
        if (ca)
            sk_X509_free(ca);
        throw AuthException{"Your certificate (" + userCert +
            ") has expired!\n"
            "Invalid since: " +
            string(time_buff->data, time_buff->length) + "."};
    }
    ...
    if (!PEM_write_bio_PrivateKey(
            key_mem, key, nullptr, nullptr, 0, nullptr, nullptr)) {
    ...
    for (int i = 0; i < X509_get_ext_count(cert); ++i) {
        ...
        if (nid == NID_proxyCertInfo &&
            userCert == userKey) { // Proxy certificate
    ...
    while (sk_X509_num(ca) > 0) {
        ...
        if (current_dn.find(tmp_dn) != string::npos) // if new DN is an
                                                     // substring of current DN,
                                                     // its a issuer of the
                                                     // proxy certificate
            current_dn = tmp_dn;
...
std::shared_ptr<communication::cert::CertificateData> GSIHandler::getCertData()
{
    if (m_proxyInitialized) {
        LOG(INFO) << "Accesing certificates via filesystem: " << m_userCertPath
                  << " " << m_userKeyPath;
        return std::make_shared<communication::cert::FilesystemCertificate>(
            m_userCertPath, m_userKeyPath,
            communication::cert::CertificateData::KeyFormat::PEM);
    }
    else {
        LOG(INFO) << "Accesing certificates via internal memory buffer.";
        return std::make_shared<communication::cert::InMemoryCertificate>(
            const_buffer(m_chainBuff->data, m_chainBuff->length),
            const_buffer(m_keyBuff->data, m_keyBuff->length),
            communication::cert::CertificateData::KeyFormat::PEM);
    }
}

std::string GSIHandler::absPathRelToHome(const string &path) const
{
    return (m_environment.userHome() / path).string();
}
...
std::string GSIHandler::getClusterHostname(const std::string &baseDomain)
{
    if (!m_context.lock()->options()->is_default_provider_hostname())
        return m_context.lock()->options()->get_provider_hostname();
    ...
    unsigned char *digest =
        MD5((const unsigned char *)DNStr, DN.length(), nullptr);
    if (!digest) {
    ...
    LOG(INFO) << "Generating cluster hostname based on user DN: " << DN
              << " || -> " << URL;
