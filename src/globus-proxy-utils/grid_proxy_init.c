...
#   define args_show_full_usage() \
    { \
        fprintf(verr, SHORT_USAGE_FORMAT \
                "%s", \
                program, \
                LONG_USAGE); \
        globus_module_deactivate_all(); \
        ...
        int                                 _passLen)
{
    ...
    if(result != GLOBUS_SUCCESS)
    {
        return globus_i_gsi_proxy_utils_print_error(result, debug, __FILE__, __LINE__,
                            "Couldn't initialize "
    ...
    if(result != GLOBUS_SUCCESS)
    {
        return globus_i_gsi_proxy_utils_print_error(
...
globus_i_gsi_proxy_utils_pwstdin_callback(
    ...
    int                                 w)
{
    int i = MIN(num, passwdLen);

    memcpy(buf, certPasswd, i);
    ...
    ...)
{
    ...
    if(debug)
    {
        globus_libc_fprintf(verr, "       %s:%d: %s", filename, line, error_string);
    }
    else 
    {
        globus_libc_fprintf(verr, "Use -debug for further information.\n");
    }
    if(error_string)
    {
