...
#define GLOBUS_I_GSI_PROXY_UTILS_PRINT_ERROR \
    return globus_i_gsi_proxy_utils_print_error(result, debug, __FILE__, __LINE__, NULL)
    ...
    FILE *                              _verr)
{
	...
	{
            X509_print_fp(vout, proxy_cert);
            print_all = GLOBUS_FALSE;
        }
        else if (strcmp(argp, "-all") == 0)
        {
            fprintf(vout, "subject  : %s\n" 
                   "issuer   : %s\n"
                   "identity : %s\n" 
                   "type     : %s\n" 
                   "strength : %d bits\n"
		   ...
		   strength,
            ...
            fprintf(vout, "%ld:%02ld:%02ld",
                   (long)(lifetime / 3600),
                   (long)(lifetime % 3600) / 60,
                   (long)lifetime % 60 );

            if (lifetime > 3600 * 24)
                fprintf(vout, "  (%.1f days)", (float)(lifetime / 3600) / 24.0);
            fprintf(vout, "\n");
            print_all = GLOBUS_FALSE;
        }
        else if ((strcmp(argp, "-valid") == 0) ||
