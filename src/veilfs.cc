...
int VeilFS::getattr(const char *path, struct stat *statbuf, bool fuse_ctx)
{
    ...
    if(attr.type() == "DIR")
    {
        ...
        // Prefetch "ls" resault
        if(fuse_ctx && VeilFS::getConfig()->getBool(ENABLE_DIR_PREFETCH_OPT)  && VeilFS::getConfig()->getBool(ENABLE_ATTR_CACHE_OPT)) {
...
int VeilFS::statfs(const char *path, struct statvfs *statInfo)
{
    ...
    int64_t quotaSize = m_fslogic->getUserQuotaSize();
    if(quotaSize == -1) {
        return -1;
    }

    int64_t filesSize = m_fslogic->getUserFilesSize();
    if(filesSize == -1) {
        return -1;
    }
    ...
    statInfo->f_blocks      = quotaSize / statInfo->f_frsize;               /* size of fs in f_frsize units */
    statInfo->f_bfree       = (quotaSize - filesSize) / statInfo->f_frsize; /* # free blocks */
    statInfo->f_bavail      = (quotaSize - filesSize) / statInfo->f_frsize; /* # free blocks for unprivileged users */
...
int VeilFS::readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset, struct fuse_file_info *fileInfo)
{
    ...
    {
        if(VeilFS::getConfig()->getBool(ENABLE_PARALLEL_GETATTR_OPT) && VeilFS::getConfig()->getBool(ENABLE_ATTR_CACHE_OPT)) {
